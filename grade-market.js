/* 정적 이력의 품목·등급·거래단위별 MA_P(최고 경락가)와 AV_P(평균 경락가)를 표시한다. */
(() => {
  ...
  function priceInfo(card, points, grade, index) {
    ...
    if(price){price.firstChild.textContent=won(point.max);const sub=price.querySelector("small");if(sub)sub.textContent=` / ${point.unit} · 평균 ${won(point.avg)}`}
    if(clock)clock.textContent=point.traded?`🕘 ${d.getMonth()+1}월 ${d.getDate()}일 · ${grade}등급 최고 경락가 ${won(point.max)} · 평균 ${won(point.avg)}`:`🕘 ${d.getMonth()+1}월 ${d.getDate()}일 · 휴장/거래 없음 · 직전 거래일 ${grade}등급 최고가 ${won(point.max)} · 평균 ${won(point.avg)}`;
  }
  function draw(card, points, grade, selected=points.length-1) {
    const svg=card.querySelector("svg");if(!svg)return;const values=points.map(p=>p.max),w=350,h=190,l=38,r=14,t=16,b=38,cw=w-l-r,ch=h-t-b,lo=Math.min(...values),hi=Math.max(...values),min=Math.floor(lo/10000)*10000,max=Math.max(min+10000,Math.ceil(hi/10000)*10000),x=i=>l+cw*i/(values.length-1),y=v=>t+(max-v)/(max-min)*ch,pts=values.map((v,i)=>`${x(i)},${y(v)}`).join(" "),ds=dates(values.length),selectedIndex=Math.max(0,Math.min(values.length-1,selected)),sx=x(selectedIndex),sy=y(values[selectedIndex]),tw=94,tx=Math.max(l,Math.min(w-tw-4,sx-tw/2)),ty=Math.max(3,sy-59);
    ...
    svg.innerHTML=`${grid}<rect class="chart-surface" data-grade-surface x="${l}" y="${t}" width="${cw}" height="${ch}"/><polygon class="chart-area" points="${l},${t+ch} ${pts} ${l+cw},${t+ch}"/><polyline class="chart-line" points="${pts}"/>${dots}<g class="chart-tooltip"><rect x="${tx}" y="${ty}" width="${tw}" height="51" rx="7"/><path d="M ${sx-5} ${ty+51} L ${sx+5} ${ty+51} L ${sx} ${ty+56}Z"/><text x="${tx+tw/2}" y="${ty+13}" text-anchor="middle">${ds[selectedIndex].getMonth()+1}/${ds[selectedIndex].getDate()}</text><text x="${tx+tw/2}" y="${ty+29}" text-anchor="middle">최고 ${won(values[selectedIndex])}</text><text x="${tx+tw/2}" y="${ty+45}" text-anchor="middle">평균 ${won(points[selectedIndex].avg)}</text></g>${labels}`;
  ...
  async function charts(card) {
    ...
    ["특","상","중"].forEach(g=>{const raw=responses.map(res=>(res.resultData||[]).find(row=>String(row.G_NAME||"").trim()===apiGrade(g))||null),first=raw.find(Boolean);if(!first)return;let last={max:number(first.MA_P),avg:number(first.AV_P),unit:String(first.U_NAME||"").trim(),traded:false};result[g]=raw.map(row=>{if(row)last={max:number(row.MA_P),avg:number(row.AV_P),unit:String(row.U_NAME||"").trim(),traded:true};return {...last}})});if(!Object.keys(result).length)throw Error("no data");return result;
  }
